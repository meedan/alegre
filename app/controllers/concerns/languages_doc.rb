...
module LanguagesDoc
  ...
  included do
    ...
    swagger_api :delete_sample do
       summary 'Delete training sample to the model'
       notes 'Use this method in order to delete training sample to the model'
       param :query, :language, :string, :required, 'Language'
       param :query, :text, :string, :required, 'Text to be deleted'
       response :ok, 'Returns ok'
       response 400, 'Parameters missing'
       response 401, 'Access denied'
    end
    ...
    swagger_api :normalize do
      ...
      authed = { CONFIG['authorization_header'] => 'test' }
      response :ok, 'Normalized text', { query: { text: ''}, headers: authed }
      response 400, 'Parameter "text" is missing', { query: nil, headers: authed }
      response 401, 'Access denied', { query: { text: 'Test' } }
    end
