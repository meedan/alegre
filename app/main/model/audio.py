...
def audio_hasher(audio,hash_size=128,average=np.median):
...
class Audio(db.Model):
  ...
  hash_value = db.Column(BIT(length=256), nullable=False, index=True)
  ...
  def from_url(url, doc_id, context={}):
    ...
    return Audio(audio_hasher(audio), doc_id, url, context)
