...
class TestAudioSimilarityBlueprint(BaseTestCase):
    ...
    def test_delete(self):
        url = 'file:///app/app/test/data/test_audio_1.mp3'
        self.model.load()
        self.model.add({"url": url, "project_media_id": 1, "conext": {'blah': 1, 'project_media_id': 12342}})
        result = self.model.delete({"url": url, "project_media_id": 1, "conext": {'blah': 1, 'project_media_id': 12342}})
        self.assertIsInstance(result, dict)
        self.assertEqual(sorted(result.keys()), ['requested', 'result'])
    ...
    def test_respond_search(self):
        ...
        with patch('app.main.lib.shared_models.audio_model.AudioModel.get_by_doc_id_or_url', ) as mock_get_by_doc_id_or_url:
            mock_get_by_doc_id_or_url.return_value = audio
            self.model.respond({"threshold": 0.7, "doc_id": "Y2hlY2stcHJvamVjdF9tZWRpYS01NTQ1NzEtdmlkZW8", "url": url, "project_media_id": 1, "command": "add", "context": {"blah": 1, 'project_media_id': 12343}})
