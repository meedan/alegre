...
- Update your [virtual memory settings](https://www.elastic.co/guide/en/opensearch/reference/current/docker.html), e.g. by setting `vm.max_map_count=262144` in `/etc/sysctl.conf`. This can also be done by the Docker UI, adjusting Resource settings to 12GB memory and 128GB of disk.  
...
![Similarity-Related HTTP requests Alegre receives from Check API](opensearch_detail.png?raw=true "Similarity-Related HTTP requests Alegre receives from Check API")
