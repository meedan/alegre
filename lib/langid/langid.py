...
class LangId:  
	...
	def __init__(self,trainPath,languages,stopwords_path):
		...
		for lang in languages:
			self.documents.append(self.readFile(trainPath+lang.lower()))
		self.initVars(trainPath)

#NOMES CONFIGURAVEIS
	def initVars(self,trainPath):
		reload(sys)  
		sys.setdefaultencoding('utf8')

	    #print doc
		if os.path.exists(trainPath+'/corpusTest.mm'):
			self.dictionary = corpora.Dictionary.load(trainPath+'/dictTest.dict')
			self.corpus = corpora.MmCorpus(trainPath+'/corpusTest.mm')
			self.tfidf = models.TfidfModel.load(trainPath+'/model.tfidf')
			...
			self.dictionary.save(trainPath+'/dict.dict')
			corpora.MmCorpus.serialize(trainPath+'/corpus.mm', self.corpus)
			self.tfidf = models.TfidfModel(self.corpus) # step 1 -- initialize a model
			self.tfidf.save(trainPath+'/model.tfidf') # same for tfidf, lda, ...
	  ...
	  for file in os.listdir(filesPath):
	   ...
	   if len(ret) == 0:
			...
			if len(sims) > 0:
				...
				lang1= self.languages[sims[0][0]]
				lang2= self.languages[sims[1][0]]
				...
				if sims[0][1] > 0:
					...
					lang = self.detectStopwords(stopwordsPath, text)
					if len(lang) > 0:
						language = lang
						ret.append ([str(self.language), 1])
				for s in sims:
					ret.append( [str(self.languages[s[0]]) , str(s[1])])
	   return ret

if __name__ == '__main__':
    l = LangId('/home/ccx/work/alegre/lib/langid/modelSentences/',['AR', 'EN', 'FA', 'HI', 'IT', 'PT', 'TL', 'AZ', 'ES', 'FR', 'ID', 'KA', 'RU', 'TR'],'/home/ccx/work/alegre/lib/langid/stopword/')
    print l.classify('um teste')
    print l.classify('cccccccc cccccccccc cccccccc')
    print l.classify('怎么感觉从奥巴马上台到现在川普崛起，米国社会的割裂更加严重了。 ')
